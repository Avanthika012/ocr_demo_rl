...
#     else:
#         main(args)
class PaddleOCRxDETxRec():

    def __init__(self,rec_model_dir,det_model_dir,rec_char_dict_path="en_dict.txt", 
                 res_path = "../results" ) -> None:

        self.args = utility.parse_args()
        self.args.rec_model_dir = rec_model_dir
        ...
        self.font_path = self.args.vis_font_path
        ...
        ### loading model
        self.modelpaddlex = TextSystem(self.args)
        ...
        if self.args.warmup:
            img = np.random.uniform(0, 255, [640, 640, 3]).astype(np.uint8)
            for i in range(10):
                res = self.modelpaddlex(img)

    def ocrx(self, image,img_name=None,manualEntryx=None):

        try:
            
            ### text detection & text recognition
            print(f"-------- img shape: {image.shape}")
            dt_boxes, rec_res, time_dict = self.modelpaddlex(image)
            self.img_count +=1

            ### to visualize and save result image
            if self.is_visualize:
                ...
                res_txt = [rec_res[i][0] for i in range(len(rec_res))]
                print(f"--------------res_txt:{res_txt}")
    ...
    def __call__(self, image,img_name=None,manualEntryx=None):
        ...
        res_img,res_txt,img_save_name = self.ocrx(image,img_name=img_name,manualEntryx=manualEntryx)
