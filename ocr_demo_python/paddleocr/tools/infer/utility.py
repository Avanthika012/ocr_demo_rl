...
def init_args():
    ...
    parser.add_argument("--det_db_unclip_ratio", type=float, default=0.5)
...
#     drop_score=0.5,
#     font_path="./doc/fonts/simfang.ttf",
#     font_size_factor=5   # Add a parameter to control the font size
# ):
#     # Validate the font_size_factor
#     if not (0.5 <= font_size_factor <= 10.0):
#         raise ValueError("font_size_factor should be between 0.5 and 5.0")
    
#     h, w = image.height, image.width
#     # Create a new image with additional space below the original image
#     new_h = h + 1200  # Increase the height to accommodate text (adjust as needed)
#     img_show = Image.new("RGB", (w, new_h), (255, 255, 255))
#     img_show.paste(image, (0, 0))

#     draw = ImageDraw.Draw(img_show)
#     font = ImageFont.truetype(font_path, int(20 * font_size_factor))

#     y_offset = h + 10  # Start drawing text below the original image with some margin

#     for idx, (box, txt) in enumerate(zip(boxes, txts)):
#         if scores is not None and scores[idx] < drop_score:
#             continue
#         text = f"{txt}" if scores is not None else txt
#         draw.text((10, y_offset), text, fill=(0, 0, 0), font=font)
#         y_offset += font.getbbox(text)[1] + 5  # Move to the next line
...
#     line_height = 100 # Height of each line with some margin
...
#         # Create a new image with additional space below the current image
...
#     h, w = image.height, image.width
#     img_top = image.copy()
#     img_bottom = np.ones((h, w, 3), dtype=np.uint8) * 255
#     random.seed(0)
...
#         img_bottom_text = draw_box_txt_fine((w, h), box, txt, font_path)
#         pts = np.array(box, np.int32).reshape((-1, 1, 2))
#         cv2.polylines(img_bottom_text, [pts], True, color, 1)
#         img_bottom = cv2.bitwise_and(img_bottom, img_bottom_text)
...
def draw_box_txt_fine(img_size, box, txt, font_path="./doc/fonts/simfang.ttf"):
    ...
    img_text = np.array(img_text, dtype=np.uint8)
    img_bottom_text = cv2.warpPerspective(
        img_text,
        M,
        img_size,
        flags=cv2.INTER_NEAREST,
